...
public class BaseDatabaseTest {
    ...
    /**
     * Helper method untuk memeriksa apakah table ada di database
     * @param tableName Nama table yang akan dicek
     * @return true jika table exists, false jika tidak
     */
    protected boolean tableExists(String tableName) throws SQLException {
        try (Statement stmt = connection.createStatement();
             var rs = stmt.executeQuery(
                     "SELECT EXISTS (SELECT FROM information_schema.tables WHERE table_name = '" + tableName.toLowerCase() + "')")) {
    ...
    /**
     * Helper method untuk mendapatkan informasi tentang table
     * @param tableName Nama table yang akan diinspeksi
     */
    protected void inspectTable(String tableName) throws SQLException {
        try (Statement stmt = connection.createStatement();
             var rs = stmt.executeQuery(
                     "SELECT column_name, data_type, is_nullable FROM information_schema.columns WHERE table_name = '" + tableName.toLowerCase() + "' ORDER BY ordinal_position")) {
            ...
            while (rs.next()) {
                ...
                logger.info("Column: " + columnName + ", Type: " + dataType + ", Nullable: " + isNullable);
            }
        }
    }
}